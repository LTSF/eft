...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton(label, col_v4, flags, ImVec2(15.5f, 15)))
        {
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec2 label_size = ImGui::CalcTextSize("CUSTOM COLOR", NULL, true);
    const ImRect check_bb(window->DC.CursorPos + ImVec2(3, 0), window->DC.CursorPos + ImVec2(5, 0) + ImVec2(label_size.y + style.FramePadding.y * 2, label_size.y + style.FramePadding.y * 2));
    ImGui::ItemSize(check_bb, style.FramePadding.y);

    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        ImGui::SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(window->DC.CursorPos + ImVec2(0, 0) + ImVec2(0, style.FramePadding.y), window->DC.CursorPos + ImVec2(4, 0) + ImVec2(0, style.FramePadding.y) + label_size);

    ImGui::GetWindowDrawList()->AddText(ImGui::GetDefaultFont(), 14, text_bb.GetTL() - ImVec2{7, 2}, ImColor(215, 215, 215, int(155 * ImGui::GetStyle().Alpha)), "CUSTOM COLOR");
